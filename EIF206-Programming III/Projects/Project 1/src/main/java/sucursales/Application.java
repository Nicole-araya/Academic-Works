...
public class Application {
    public static void main(String[] args) {
        ...
        sucursales.presentation.AcercaDe.View acercaView = new sucursales.presentation.AcercaDe.View();

        mainView.getPanel().add("Empleados",empleadosView.getPanel());
        mainView.getPanel().add("Sucursales",sucursalesView.getPanel());
        mainView.getPanel().add("Acerca De", acercaView.getPanel());

        window = new JFrame();
        window.setSize(400,300);
        window.setExtendedState(JFrame.MAXIMIZED_BOTH);
        window.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
        window.setTitle("Nicole-Ariana_P3_Proyecto1");
