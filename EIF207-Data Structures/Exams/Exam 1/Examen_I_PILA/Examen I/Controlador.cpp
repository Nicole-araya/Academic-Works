...
{
    for (int h = disco-1; h >= 0; h--) {
        torre1.push(new float(pow((h + 9),(h + 1) ) * (h + 4)));
    }
}
...
void Controlador::imprimirSolucion(int disco, int t1, int t2, int t3, bool flag){
	...
	if(disco == 1){
		...
		imprimirSolucion(disco - 1, t1, t3, t2,flag);
		if (flag) { cout << "Disco de la torre " << t1 << " a torre " << t3 << endl; }
		minMovimienos++;
		imprimirSolucion(disco - 1, t2, t1, t3,flag);
		
    }
	
}

bool Controlador::comparaTop(Pila<float>& n1, Pila<float>& n2)
{
	if (n2.empty()) return false;
	return *n1.top() > *n2.top();  //n1 es el que viene
	...
	while (opcion != 7) {
		...
		try {
			...
			case 1:
				...
				if (comparaTop(torre1, torre2)) throw compara;

				x = torre1.top();
				torre2.push(new float(*x));
				torre1.pop();
				movimientos++;
